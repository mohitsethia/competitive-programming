...
void solve(){
    ...
    int last = 0;
    for(int i = 0; i < m; i++){
        if(i && v[i].first.first == v[i-1].first.first && v[i].first.second == v[i-1].first.second) continue;
            bits |= v[i].second;
        last = max(last, v[i].first.second);
    }
    cout << ((bits * (fast_pow(2, n-1)%mod)%mod) + mod)%mod << "\n";
}
